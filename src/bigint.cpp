...
bigint_err bigint_init(bigint* a) {
    ...
    if (a->d == nullptr) return BIGINT_ERR_MEMORY;
    a->used = 0;
...
#define BIGINT_INIT_INT(name, set_func, type)                                   \
    bigint_err name(bigint* a, type b) {                                        \
        bigint_err err;                                                         \
...
bigint_err _bigint_add(const bigint* a, const bigint* b, bigint* c) {
...
// assumes |a| > |b|
bigint_err _bigint_sub(const bigint* a, const bigint* b, bigint* c) {
